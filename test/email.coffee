...
describe "Email", ->
	...
	it "should error because there is no 'Message.Body.Html.Data'", (done) ->
		opts = _.cloneDeep(options)
		opts.Message.TemplateType = 'jade'
		email = new Email(opts, credentials)
		email.send (err, result) ->
			should.exist(err)
			should.not.exist(result)
	...
	# JADE
	it "should not send Jade template since jade cannot parse plain text", (done) ->
		opts = _.cloneDeep(options)
		opts.Message.Subject.Data = 'Jade Email Template Test'
		opts.Message.TemplateType = 'jade'
		opts.Message.Body.Html.Data = conf.templates.jade.html
		opts.Message.Body.Text.Data = conf.templates.jade.text
		email = new Email(opts, credentials)
		email.send (err, result) ->
			should.exist(err)
			should.not.exist(result)
	...
	it "should send Jade template", (done) ->
		opts = _.cloneDeep(options)
	...
	it "should send Jade template from uri", (done) ->
		opts = _.cloneDeep(options)

		opts.Message.Subject.Data = 'Jade Email Template from URI Test'
		opts.Message.Body.Html.Data = conf.templateUris.jade.html
		opts.Message.Body.Text.Data = null
		email = new Email(opts, credentials)
		email.send (err, result) ->
			should.not.exist(err)
